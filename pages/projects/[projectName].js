...
const ProjectPage = () => {
    ...
    const project = projects.find(proj => proj.url === projectName);
                        ...
                        modules={[Autoplay, Keyboard, Pagination, Navigation]} className=" cursor-default w-screen h-screen">
                        {
                            project.allImages.map((image, index) => (
                                <SwiperSlide key={index + 1}>
                                    <Image src={image} width={1500} height={1500} alt={`Image ${index + 1}`} className='max-h-screen min-h-screen md:object-contain ' placeholder='blur'></Image>
                                </SwiperSlide>
                            ))
                        }
                    </Swiper>
